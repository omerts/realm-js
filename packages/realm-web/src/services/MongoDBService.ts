...
export class WatchStream<T extends Document> {
    ...
    feedSse(sse: ServerSentEvent) {
        ...
        if (sse.eventType === undefined || sse.eventType.length == 0 || sse.eventType == "message") {
            ...
            //this.error.emplace(app::make_error_code(ServiceErrorCode::unknown), std::string(sse.data));
            // XXX
            ...
            try {
                ...
                //this.error.emplace(app::make_error_code(app::service_error_code_from_string(
...
class MongoDBCollection<T extends Document>
    ...
    async* watch(
        ...
        const reply = await this.fetcher.fetch(
            await this.fetcher.makeStreamingRequest(
                "watch",
                cleanArgs([args]),
                this.serviceName));
